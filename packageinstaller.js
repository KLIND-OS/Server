    ...
    static installPackage(win) {
        ...
        LowLevelApi.Packages.install(repository, packageName, response => {
            if (response == "error") {
                ...
                win.querySelector(".helptext").textContent = "Aj, nastala chyba při instalaci balíčku. Zkontrolujte že máte správně vybraný repozitář a že máte správný název balíčku.";
            }
            else if (response == "success") {
                win.querySelector(".helptext").textContent = "Balíček byl úspěšně nainstalován!";
